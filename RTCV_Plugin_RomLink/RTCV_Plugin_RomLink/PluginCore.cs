...
namespace ROMLINK
{
    ...
    public class ROMLINK : IPlugin, IDisposable
    {
        //-------[ Plugin metadata ]-------

        // >>> Make sure you rename BOTH the namespace and class (Very important)
        public string Description => "Lets you hook, import, export roms through a plugin.";
        ...
        public string Name => nameof(ROMLINK).Replace("_"," ");
        ...
        public bool Stop()
        {
            if (ROMLINK.CurrentSide == RTCSide.Client && !S.ISNULL<PluginForm>() && !S.GET<PluginForm>().IsDisposed)
            {
